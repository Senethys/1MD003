...
import processing.sound.*; //For sound effects.
import javax.swing.*; //Dialog before the game is run
import java.awt.*;

SoundFile cheerFile; //Start
SoundFile gameOverFile; //End
SoundFile woodFile; //Collision 
...
//A solution using dynamic size
  ...
  //This will only work with setup(). Settings() are used for putting variables in size(x,y) because setup does not permit that.
...
//This creates a new tile and puts it in a random empty spot on the board 
void spawn()  
{ 
...
void draw() { 
  ...
  //Paint an empty board of the game. 
  //    x,y,  w  , h  , corner,   colour 
  rectt(0,0,width,height,10,color(gridColor)); 
   
  //Create rectangles in on the board 
  for (int j = 0 ; j < side; j++)  
    for (int i = 0 ; i < side; i++) { 
...
//Algortihm for movement. Where is the collision here? 
int[][] go(int dy, int dx, boolean updatescore) { 
   
      //Make a copy of the board so it can be modified 
